...
const dateYMDGeter = function (d: Date) {
  return [d.getFullYear(), d.getMonth(), d.getDate()];
};
const nowDate = new Date();
const [yearNow, monthNow, dayNow] = dateYMDGeter(nowDate);
...
let _checkedDate = { year: yearNow, month: monthNow, day: dayNow };
const state = {
  ...
  nextMonth: 0, // 下个月的月份
  // checkedDate: _checkedDate, // 被选中的日期
  checkedDateArr: [[yearNow, monthNow, dayNow]], // (多选模式下)被选中的日期
...
export type CalendarProps = {
  ...
  dateChangeCallback?: (date?: IDate | any) => void;
...
type State = {
  ...
  checkedDate?: null | { year?: number; month?: number; day?: number };
} & typeof state;
...
> {
  ...
  public state: State = { ...state, checkedDate: _checkedDate };
  ...
  clearCheckedDate = () => {
    const { isShowWeek, transitionDuration } = this.state;
    this.setState({
      checkedDate: null,
      checkedDateArr: [],
    });
  ...
  nextSomeDayCk = (days: number, isPast?: boolean, includeToday?: boolean) => {
    const { disabledDate } = this.props;
    let factor = (isPast ? -1 : 1) * 24 * 3.6e6;
    const nowTimestamp = nowDate.getTime();
    let _timestamp = includeToday ? nowTimestamp : nowTimestamp + factor;
    let retArr: Date[] = [];
    for (let index = 0; index < days; index++) {
      retArr.push(new Date(_timestamp));
      _timestamp += factor;
    }
    retArr = retArr.filter((v) => !disabledDate(v));

    const { isShowWeek, transitionDuration } = this.state;
    this.setState(
      {
        checkedDateArr: retArr.map((v) => dateYMDGeter(v)),
        yearOfCurrentShow: yearNow,
        monthOfCurrentShow: monthNow,
      },
      () => {
  ...
  currentMonthCk = (
    onlyWorkday?: boolean,
    filterFn?: (checkedDate: any) => boolean | void
  ) => {
    const { disabledDate } = this.props;
    const { checkedDate, isShowWeek, transitionDuration } = this.state;
    let step = 24 * 3.6e6;
    const firstDayimestamp = new Date(`${yearNow}/${monthNow + 1}/1`).getTime();
    let _timestamp = firstDayimestamp;
    let retArr: Date[] = [];
  ...
  showWeek = (checkedDate?: any) => {
    ...
    checkedDate =
      ...
      (_checkedDate?.day
        ? _checkedDate
        : { day: dayNow, year: yearNow, month: monthNow });
  ...
  doSlide = (isRight: boolean) => {
    const { touch, isShowWeek, transitionDuration } = this.state;
    const { slideChangeCallback } = this.props;
    this.setState({ currentChangeIsScroll: true, isTouching: false }, () => {
      if (isRight) {
        slideChangeCallback && slideChangeCallback("right");
        this.getLastMonth();
        if (isShowWeek) {
  ...
  isCheckedDay = (date: IDate): boolean => {
    ...
    const { checkedDate, checkedDateArr } = this.state;
    if (!multiple) {
      if (!checkedDate?.day) {
        return false;
      }
      ...
      return this.indexOfMultipleCheckedSet([year, month, day]) > -1;
    }
  };

  indexOfMultipleCheckedSet = (dateYMDStr: number[]): number => {
    const { checkedDateArr } = this.state;
    const strArr = checkedDateArr.map((v) => v.toString());
    return strArr.indexOf(dateYMDStr.toString());
  };
    ...
    this.setState(
      {
        ...
        checkedDateArr: multiple ? _checkedDateArr : checkedDateArr,
      },
      () => {
        const {
          lastMonth,
          ...
          >
            {(daySlot &&
              daySlot(date, {
                ...
                originalTxt: this.isFirstDayOfMonth(date, mIndex)
    ...
    return show ? (
      ...
      >
        <div className="adm-calendar-title">
          <a className="lt-op">
            <span onTouchEnd={() => this.doSlide(true)}>
              {monthTitleActionSlot?.[0] || <>上一{isShowWeek ? "周" : "月"}</>}
            </span>
          </a>
          <p className="calendar-month-title">
            {`${yearOfCurrentShow}年${
              language.MONTH && language.MONTH[monthOfCurrentShow]
            }`}
          </p>
          <a className="gt-op">
            <span onTouchEnd={() => this.doSlide(false)}>
              {monthTitleActionSlot?.[1] || <>下一{isShowWeek ? "周" : "月"}</>}
            </span>
          </a>
        </div>
        {weekNode}
